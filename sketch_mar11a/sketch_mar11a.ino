...
void setup() {
  ...
  // image file.
  park = TFTscreen.loadImage("park.bmp");
  if (!park.isValid()) {
    Serial.println(F("error while loading park.bmp"));
  }
}

void loop() {
  // don't do anything if the image wasn't loaded correctly.
  if (park.isValid() == false) {
    return;
  }
  ...
  int x = random(TFTscreen.width() - park.width());
  int y = random(TFTscreen.height() - park.height());

  // draw the image to the screen
  TFTscreen.image(park, x, y);
