...
extEEPROM eep(kbits_16, 8, 16);         //device size, number of devices, page size
...
void setup(void)
{
    ...
    Serial << endl << F("Send button to start...") << endl;
    while (!Serial.available()) delay(10);    //wait for button push

    uint8_t chunkSize = 64;    //64 this can be changed, but must be a multiple of 4 since we're writing 32-bit integers
...
for (uint32_t addr = 0; addr < totalKBytes * 1024; addr += chunk) {
  ...
  // Print 8 lines....  
  for (uint8_t c = 0; c < chunk; c +=8) {
    // Of 8 values, comma separated
      for(uint8_t l=0;l<8;l++){
        //sprintf(buff,"%%dX",data[l+c]);
        //Serial.print(buff);
        if(data[l+c] < 0x10) Serial.print("0");
        Serial.print(data[l+c],HEX);
        //Serial.print(",");
      }
    //Serial.println();
      //Serial.print(addr+c);
     // Serial.print(":");
      //Serial.print(data[c+0],HEX);
      //Serial.print(data[c+1],HEX);
      //Serial.print(data[c+2],HEX);
      //Serial.println(data[c+3],HEX);
      
      //Serial <<  addr << F(":") << data[c+0] << data[c+1] << data[c+2] << data[c+3] << endl;
   }
